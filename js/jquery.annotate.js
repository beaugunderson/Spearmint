...
(function($){
   ...
   $.fn.addAnnotations = function(annotationCallback, annotations, options) {
      ...
      $.each(annotations, function() {
         ...
         if (isFloat(this.x) && isFloat(this.y)) {
            left = (this.x * $(container).width()) - ($(element).xOffset(options.xPosition));
            top = (this.y * options.height) - ($(element).yOffset(options.yPosition));
         } else if (this.x < 0 && this.y < 0) {
            left = (this.x + $(container).width()) - ($(element).xOffset(options.xPosition));
            top = (this.y + options.height) - ($(element).yOffset(options.yPosition));
         } else {
            left = this.x - $(element).xOffset(options.xPosition);
            top = this.y - $(element).yOffset(options.yPosition);
         }
